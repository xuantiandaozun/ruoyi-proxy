...
Agent 会根据节点角色与现场环境自适应运维，**不默认**所有服务器都使用蓝绿代理：

| 场景 | Agent 行为 |
|------|-----------|
| **蓝绿代理模式** | 8000 在跑或用户确认需要代理 → 使用 deploy/switch/proxy-status 等命令 |
| **通用运维模式** | Node/Python/Docker/单实例等 → 先探查进程/端口/容器/systemd，不强行检查蓝绿 |
| **Hub 节点** | 网关职责（Token、Spoke 列表、AI 转发）与本机业务运维分开；协助 Spoke 时参考节点档案 |
| **Spoke 节点** | 常仅作 AI 运维 CLI；代理未运行不等于故障，不确定时会询问用户 |
...
非标准项目（无 jar、无蓝绿）时，Agent 会检测项目类型并适配 `service-*.sh` 控制脚本。
...
| 工具 | 说明 | 是否需要确认 |
|------|------|:----------:|
| `get_status` | 查询服务状态（按项目类型自适应健康检查；代理未运行时不判为故障） | ❌ |
| `get_config` | 查看代理与服务配置 | ❌ |
| `get_logs` | 读取服务日志 | ❌ |
| `get_system_info` | 查看 CPU/内存/磁盘等 | ❌ |
...
| 节点 | 检查范围 |
|------|---------|
| **Hub** | 基础环境、:8000/:8001 网关、AI 配置、/__hub__/ Nginx 路由 |
| **Spoke** | 基础环境、Hub 连接；8000 代理未监听时标为跳过（非必检） |
| **通用** | 基础环境；完整业务健康检查由 Agent 按部署模式现场探查 |

Spoke 或非蓝绿服务器上，可自然语言说「帮我自检一下服务器」，Agent 会先判断是否需要蓝绿代理，再选择 systemd/docker/端口等检查手段。
...
>
> **注意**：Hub 与各 Spoke 的业务服务器不一定使用蓝绿代理。Spoke 常仅作 AI 运维 CLI；Agent 会读取 `spoke_profile.json`（Spoke）或 `/hub-status` 档案（Hub）了解项目类型，自检时按实际环境探查，不默认检查 proxy-status 或 `/actuator/health`。
