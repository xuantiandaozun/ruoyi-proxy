...
install_network_tools() {
    echo -e "\n${BLUE}========================================${NC}"
    echo -e "${BLUE}[4.5/6] 安装网络工具${NC}"
    echo -e "${BLUE}========================================${NC}"
    
    # 检查并安装 nc (netcat)
    if ! check_installed nc; then
        echo -e "${CYAN}安装 netcat...${NC}"
        sudo yum install -y nmap-ncat
        
        if check_installed nc; then
            echo -e "${GREEN}✓ netcat 安装完成${NC}"
        else
            echo -e "${YELLOW}⚠ netcat 安装失败，部分功能可能受限${NC}"
        fi
    else
        echo -e "${YELLOW}netcat 已存在，跳过安装${NC}"
    fi
    
    # 检查并安装 curl
    if ! check_installed curl; then
        echo -e "${CYAN}安装 curl...${NC}"
        sudo yum install -y curl
        
        if check_installed curl; then
            echo -e "${GREEN}✓ curl 安装完成${NC}"
        else
            echo -e "${YELLOW}⚠ curl 安装失败${NC}"
        fi
    else
        echo -e "${YELLOW}curl 已存在，跳过安装${NC}"
    fi
}
